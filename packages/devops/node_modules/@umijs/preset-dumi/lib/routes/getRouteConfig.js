...
var _default = (api, opts) => {
  ...
  if (opts.routes) {
    ...
    config[0].routes = opts.routes.map((_ref) => {
    ...
    opts.resolve.includes.forEach(item => {
      ...
      if (_fs().default.existsSync(docsPath) && _fs().default.statSync(docsPath).isDirectory()) {
        config[0].routes.push(...(0, _getRouteConfigFromDir.default)(docsPath, opts));
      }
    });
  } // decorate standard umi routes


  config[0].routes = (0, _decorator.default)(config[0].routes, opts, api);
  return config;
};
