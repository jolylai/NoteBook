...
function _default(api) {
  ...
  api.onPatchRoutesBefore(({
    ...
    if (!parentRoute) {
      const opts = mergeUserConfig(defaultOpts, api); // save umi api & opts into context

      (0, _context.init)(api, opts);
      const result = (0, _getRouteConfig.default)(api, opts); // clear original routes
  ...
  api.chainWebpack(config => {
    ...
    config.module.rule('dumi').test(/\.md$/).use('babel-loader').loader(babelLoader.loader).options(babelLoader.options).end().use('dumi-loader').loader(require.resolve('../loader')).options({
      previewLangs: opts.resolve.previewLangs
  ...
  api.addTmpGenerateWatcherPaths(() => {
    const opts = mergeUserConfig(defaultOpts, api);
    return [...opts.resolve.includes.map(key => _path().default.join(api.paths.cwd, key, '**/*.md'))];
  }); // TODO: CLI help info
