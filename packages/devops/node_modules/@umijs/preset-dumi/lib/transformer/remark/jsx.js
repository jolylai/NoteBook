...
function hasSubClassName(className, subCls) {
  return (className || []).find(cls => cls.indexOf(subCls) > -1);
}

const textVisitor = (node, ancestors) => {
  const parentNode = ancestors[ancestors.length - 1];
  const closestCodeAncestor = ancestors.slice().reverse().find(ancestor => ancestor.tagName === 'code'); // escape { & } for JSX
  ...
  if (closestCodeAncestor && (hasSubClassName(closestCodeAncestor.properties.className, 'language-') || ancestors[ancestors.length - 2].tagName === 'pre')) {
