  ...
  return ast => {
    (0, _unistUtilVisit().default)(ast, 'element', (node, i, parent) => {
      if ((0, _hastUtilIsElement().default)(node, 'img') && (0, _hastUtilHasProperty().default)(node, 'src') && isRelativeUrl(node.properties.src)) {
        parent.children[i] = {
          type: 'raw',
          value: `<img src={require('${_path().default.join(_path().default.parse(this.data('fileAbsPath')).dir, node.properties.src)}')}>`
        };
      }
    });
      ...
      if (typeof node.value === 'string') {
        node.value = node.value.replace(/<img.*?\/?>/g, tag => {
          ...
          if (isRelativeUrl(src)) {
            return `<img src={require('${_path().default.join(_path().default.parse(this.data('fileAbsPath')).dir, src)}')} {...${JSON.stringify(inheritAttrs)}}>`;
          }
